...
export default defineCommand({
  ...
  async run({ args }) {
    await runTool({
      file: args.file,
      toolName: 'set_fill',
      args: { id: args.id, color: args.color },
      write: args.write,
      output: args.output,
      json: args.json,
      quiet: args.quiet,
      formatResult: (r) => ok(`Set fill on ${(r as { id: string }).id} to ${args.color}`),
    })
  },
})
